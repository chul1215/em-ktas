...
- **단일 파일 구조**: HTML + CSS + JS가 모두 인라인으로 포함된 SPA (~1000줄)
...
### 참조 데이터 파일 (index.html에서 직접 로드하지 않음)
- `dashboard/js/data.js` — 더 상세한 필드명을 사용하는 참조용 JS. `index.html`의 `D` 객체와 **동일한 수치** 유지 필수
- `dashboard/css/style.css` — 참조용 CSS 파일. `index.html`은 인라인 `<style>`만 사용하므로 이 파일은 실제로 적용되지 않음
...
### 로그인 게이트
- 접속 시 전체화면 `#loginOverlay` 표시 (z-index:9999)
- 허용 사번: `VALID_IDS = ['14470', '17548']` — JS 상수로 관리
- 올바른 사번 입력 시 overlay `display:none` → 대시보드 노출
- 입력 필드: `type="password"` + `inputmode="numeric"`, maxlength=5

### 원본 데이터 (`data/`)
- `data/now_data/` — 현재 분석 기준 xlsx + 분석 결과 md 파일 (`대전병원_분석결과.md`, `유성병원_분석결과.md`, CSV 등). 원본 xlsx는 수정하지 않음. 분석결과 MD/CSV는 대시보드 수치 변경 시 함께 갱신
- `data/pre_data/` — 이전 분석 기간 원본 xlsx. 직접 수정하지 않음
...
- **KPI 목표**: `D[H].kpi` (병원별 상이) — 대전 `{1:100,2:95,3:80,4:50,5:15}`, 유성 `{1:95,2:95,3:80,4:50,5:15}`
- **KTAS 색상**: `D.kc` = `{1:'#5BA8FF', 2:'#38D9F5', 3:'#A78BFA', 4:'#FBBF24', 5:'#F472B6'}`
- **수익 시나리오**: 2종 (외래 `out` / 입원연계 `adm`)
  - 외래 KPI 수익 = (전체건 - 입원건) × KPI목표율 × 단가
  - 입원 KPI 수익 = 입원건 × 100% × 입원단가
  - 단가: 대전 `{out:144040, adm:333020}` / 유성 `{out:158570, adm:514830}`
...
9. `brief/presentation/대전_발표문.md`
10. `brief/presentation/유성_발표문.md`
